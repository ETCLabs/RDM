...
etcpal_error_t rdmctl_pack_command(const RdmCommand *cmd_data, RdmBuffer *buffer)
{
  ...
  *cur_ptr++ = rdm_length;
  etcpal_pack_16b(cur_ptr, cmd_data->dest_uid.manu);
  cur_ptr += 2;
  etcpal_pack_32b(cur_ptr, cmd_data->dest_uid.id);
  cur_ptr += 4;
  etcpal_pack_16b(cur_ptr, cmd_data->source_uid.manu);
  cur_ptr += 2;
  etcpal_pack_32b(cur_ptr, cmd_data->source_uid.id);
  cur_ptr += 4;
  *cur_ptr++ = cmd_data->transaction_num;
  *cur_ptr++ = cmd_data->port_id;
  *cur_ptr++ = 0;
  etcpal_pack_16b(cur_ptr, cmd_data->subdevice);
  cur_ptr += 2;
...
etcpal_error_t rdmctl_unpack_response(const RdmBuffer *buffer, RdmResponse *resp)
{
  ...
  cur_ptr += 4;
  resp->source_uid.manu = etcpal_upack_16b(cur_ptr);
  cur_ptr += 2;
  resp->source_uid.id = etcpal_upack_32b(cur_ptr);
  cur_ptr += 4;
  resp->transaction_num = *cur_ptr++;
  resp->resp_type = (rdm_response_type_t)*cur_ptr++;
  resp->msg_count = *cur_ptr++;
  resp->subdevice = etcpal_upack_16b(cur_ptr);
  cur_ptr += 2;
