...
parser = argparse.ArgumentParser(description="Generate the RDM documentation")
...
if os.getenv("BUILD_REASON", "IndividualCI") == "PullRequest":
    ...
    output_dir = "build/{}/docs/stage/{}".format(
        os.getenv("GH_REPO_NAME", "RDM"),
        os.getenv("SYSTEM_PULLREQUEST_PULLREQUESTNUMBER"),
    )
elif args.release:
    project_number = args.release
    (vers_maj, vers_min, _) = args.release.split(".")
    output_dir = "build/{}/docs/v{}".format(os.getenv("GH_REPO_NAME", "RDM"), f"{vers_maj}.{vers_min}")
else:
    project_number = "HEAD (unstable)"
    output_dir = "build/{}/docs/head".format(os.getenv("GH_REPO_NAME", "RDM"))
