...
TEST(Message, PackCommandInvalid)
{
  ...
  // Invalid arguments
  EXPECT_EQ(
      rdm_pack_command(nullptr, sensor_def_pair.cmd_data(), sensor_def_pair.cmd_data_size(), buf.data(), buf.size()),
      kEtcPalErrInvalid);
  EXPECT_EQ(rdm_pack_command(&sensor_def_pair.cmd_header, sensor_def_pair.cmd_data(), sensor_def_pair.cmd_data_size(),
                             nullptr, buf.size()),
            kEtcPalErrInvalid);
  EXPECT_EQ(rdm_pack_command(&sensor_def_pair.cmd_header, sensor_def_pair.cmd_data(), sensor_def_pair.cmd_data_size(),
                             buf.data(), 0),
  ...
  // Buffer too short
  EXPECT_EQ(rdm_pack_command(&sensor_def_pair.cmd_header, sensor_def_pair.cmd_data(), sensor_def_pair.cmd_data_size(),
                             buf.data(), sensor_def_pair.packed_cmd_size() - 1),
            kEtcPalErrBufSize);
}

// TODO pack response
// TODO pack response invalid
// TODO unpack command
// TODO unpack command invalid
// TODO unpack response
// TODO unpack response invalid

TEST(Message, BufPackCommandInvalid)
{
  RdmCommandHeader cmd_header = rdmtest::GetSupportedParams().cmd_header;
  RdmBuffer        buffer{};
  uint8_t          data[2]{};

  EXPECT_EQ(rdm_buf_pack_command(nullptr, data, 2, &buffer), kEtcPalErrInvalid);
  EXPECT_EQ(rdm_buf_pack_command(&cmd_header, data, 2, nullptr), kEtcPalErrInvalid);

  // Invalid command class
  cmd_header.command_class = (rdm_command_class_t)0xff;
  EXPECT_EQ(rdm_buf_pack_command(&cmd_header, data, 2, &buffer), kEtcPalErrInvalid);
...
TEST(Message, BufPackCommandTooLong)
{
  RdmCommandHeader cmd_header = rdmtest::GetSupportedParams().cmd_header;
  RdmBuffer        cmd_buf;
  ...
  EXPECT_EQ(rdm_buf_pack_command(&cmd_header, data, RDM_MAX_PDL, &cmd_buf), kEtcPalErrOk);
  EXPECT_EQ(cmd_buf.data_len, RDM_MAX_PDL + RDM_HEADER_SIZE + 2);

  // One over the maximum should fail
  EXPECT_NE(rdm_buf_pack_command(&cmd_header, data, RDM_MAX_PDL + 1, &cmd_buf), kEtcPalErrOk);
}

TEST(Message, BufPackCommandWithData)
{
  const auto sensor_def_pair = rdmtest::GetSensorDefinition();
  RdmBuffer  cmd_buf;

  ASSERT_EQ(rdm_buf_pack_command(&sensor_def_pair.cmd_header, sensor_def_pair.cmd_data(),
                                 sensor_def_pair.cmd_data_size(), &cmd_buf),
            kEtcPalErrOk);
...
// rdm_buf_pack_command() should be usable to pack a command with zero-length data.
TEST(Message, BufPackCommandEmpty)
{
  const auto reset_dev_pair = rdmtest::SetResetDevice();
  RdmBuffer  cmd_buf;

  ASSERT_EQ(rdm_buf_pack_command(&reset_dev_pair.cmd_header, nullptr, 0, &cmd_buf), kEtcPalErrOk);
...
TEST(Message, BufPackResponseInvalid)
{
  RdmCommandHeader cmd_header = rdmtest::GetSupportedParams().cmd_header;
  RdmBuffer        buffer{};
  uint8_t          data[2]{};

  EXPECT_EQ(rdm_buf_pack_response(nullptr, 0, data, 2, &buffer), kEtcPalErrInvalid);
  EXPECT_EQ(rdm_buf_pack_response(&cmd_header, 0, data, 2, nullptr), kEtcPalErrInvalid);

  // Invalid command class
  cmd_header.command_class = (rdm_command_class_t)0xff;
  EXPECT_EQ(rdm_buf_pack_response(&cmd_header, 0, data, 2, &buffer), kEtcPalErrInvalid);
...
TEST(Message, BufPackResponseTooLong)
{
  RdmCommandHeader                     cmd_header = rdmtest::GetSupportedParams().cmd_header;
  ...
  // Equal to maximum should succeed
  EXPECT_EQ(rdm_buf_pack_response(&cmd_header, 0, pd.data(), RDM_MAX_PDL, &response_buf), kEtcPalErrOk);
  EXPECT_EQ(response_buf.data_len, RDM_MAX_PDL + RDM_HEADER_SIZE + 2);

  // One over the maximum should fail
  EXPECT_NE(rdm_buf_pack_response(&cmd_header, 0, pd.data(), RDM_MAX_PDL + 1, &response_buf), kEtcPalErrOk);
}

TEST(Message, BufPackResponseWithData)
{
  const auto identify_pair = rdmtest::GetIdentifyDevice();
  RdmBuffer  response_buf;

  ASSERT_EQ(rdm_buf_pack_response(&identify_pair.cmd_header, identify_pair.RespMessageCount(),
                                  identify_pair.resp_data(), identify_pair.resp_data_size(), &response_buf),
            kEtcPalErrOk);
...
// rdm_buf_pack_response() should be usable to pack a response with zero-length data.
TEST(Message, BufPackResponseEmpty)
{
  const auto reset_dev_pair = rdmtest::SetResetDevice();
  RdmBuffer  response_buf;

  ASSERT_EQ(
      rdm_buf_pack_response(&reset_dev_pair.cmd_header, reset_dev_pair.RespMessageCount(), nullptr, 0, &response_buf),
      kEtcPalErrOk);
...
TEST(Message, BufPackOverflowResponseInvalid)
{
  RdmCommandHeader cmd_header = rdmtest::GetSupportedParams().cmd_header;
  RdmBuffer        buffer{};

  EXPECT_EQ(rdm_buf_pack_overflow_response(nullptr, 0, kRdmNRUnsupportedCommandClass, &buffer), kEtcPalErrInvalid);
  EXPECT_EQ(rdm_buf_pack_overflow_response(&cmd_header, 0, kRdmNRUnsupportedCommandClass, nullptr), kEtcPalErrInvalid);

  // Invalid command class
  cmd_header.command_class = (rdm_command_class_t)0xff;
  EXPECT_EQ(rdm_buf_pack_overflow_response(&cmd_header, 0, kRdmNRUnsupportedCommandClass, &buffer), kEtcPalErrInvalid);
...
TEST(Message, BufPackOverflowResponse)
{
  ...
  ASSERT_EQ(rdm_buf_pack_overflow_response(&overflow.cmd_header, overflow.resp_data(), 230, &response_buf),
            kEtcPalErrOk);
...
TEST(Message, BufPackNackResponseInvalid)
{
  RdmCommandHeader cmd_header = rdmtest::GetSupportedParams().cmd_header;
  RdmBuffer        buffer{};

  EXPECT_EQ(rdm_buf_pack_nack_response(nullptr, 0, kRdmNRUnsupportedCommandClass, &buffer), kEtcPalErrInvalid);
  EXPECT_EQ(rdm_buf_pack_nack_response(&cmd_header, 0, kRdmNRUnsupportedCommandClass, nullptr), kEtcPalErrInvalid);

  // Invalid command class
  cmd_header.command_class = (rdm_command_class_t)0xff;
  EXPECT_EQ(rdm_buf_pack_nack_response(&cmd_header, 0, kRdmNRUnsupportedCommandClass, &buffer), kEtcPalErrInvalid);
...
TEST(Message, BufPackNackResponse)
{
  const auto nack_pair = rdmtest::SetResetDeviceNackResponse();
  RdmBuffer  response_buf;

  ASSERT_EQ(rdm_buf_pack_nack_response(&nack_pair.cmd_header, 0, nack_pair.NackReason(), &response_buf), kEtcPalErrOk);
...
TEST(Message, BufPackTimerResponseInvalid)
{
  RdmCommandHeader cmd_header = rdmtest::GetSupportedParams().cmd_header;
  RdmBuffer        buffer{};

  EXPECT_EQ(rdm_buf_pack_timer_response(nullptr, 0, 200, &buffer), kEtcPalErrInvalid);
  EXPECT_EQ(rdm_buf_pack_timer_response(&cmd_header, 0, 200, nullptr), kEtcPalErrInvalid);

  // Invalid command class
  cmd_header.command_class = (rdm_command_class_t)0xff;
  EXPECT_EQ(rdm_buf_pack_timer_response(&cmd_header, 0, 200, &buffer), kEtcPalErrInvalid);
  ...
  EXPECT_EQ(rdm_buf_pack_timer_response(&cmd_header, 0, 0, &buffer), kEtcPalErrInvalid);
  EXPECT_EQ(rdm_buf_pack_timer_response(&cmd_header, 0, std::numeric_limits<unsigned int>::max(), &buffer),
            kEtcPalErrInvalid);
}

TEST(Message, BufPackTimerResponse)
{
  const auto timer_pair = rdmtest::GetLampStrikesTimerResponse();
  RdmBuffer  response_buf;

  unsigned int delay_ms = timer_pair.AckTimerDelay();
  ASSERT_EQ(rdm_buf_pack_timer_response(&timer_pair.cmd_header, 0, delay_ms - 50, &response_buf), kEtcPalErrOk);
...
TEST(Message, BufPackFullResponseInvalid)
{
  const auto overflow = rdmtest::GetSupportedParamsWithOverflow();
  RdmBuffer  response_buf;

  // Invalid parameters
  EXPECT_EQ(rdm_buf_pack_full_response(nullptr, overflow.resp_data(), 200, &response_buf, 1), kEtcPalErrInvalid);
  EXPECT_EQ(rdm_buf_pack_full_response(&overflow.cmd_header, overflow.resp_data(), 200, nullptr, 1), kEtcPalErrInvalid);
  EXPECT_EQ(rdm_buf_pack_full_response(&overflow.cmd_header, overflow.resp_data(), 200, &response_buf, 0),
            kEtcPalErrInvalid);

  // Short buf
  EXPECT_EQ(rdm_buf_pack_full_response(&overflow.cmd_header, overflow.resp_data(), overflow.overflow_resp_data_size(),
                                       &response_buf, 1),
            kEtcPalErrBufSize);
}

TEST(Message, BufPackFullResponseSingleZeroData)
{
  const auto reset_device = rdmtest::SetResetDevice();
  RdmBuffer  response{};

  ASSERT_EQ(rdm_buf_pack_full_response(&reset_device.cmd_header, nullptr, 0, &response, 1), kEtcPalErrOk);
...
TEST(Message, BufPackFullResponseSingleWithData)
{
  const auto tcp_comms_status = rdmtest::GetTcpCommsStatus();
  RdmBuffer  response{};

  ASSERT_EQ(rdm_buf_pack_full_response(&tcp_comms_status.cmd_header, tcp_comms_status.resp_data(),
...
TEST(Message, BufPackFullResponseOverflow)
{
  const auto             overflow = rdmtest::GetSupportedParamsWithOverflow();
  ...
  ASSERT_EQ(rdm_buf_pack_full_response(&overflow.cmd_header, overflow.resp_data(), overflow.overflow_resp_data_size(),
...
TEST(Message, BufAppendParameterDataInvalid)
{
  const auto supported_params = rdmtest::GetSupportedParams();
  RdmBuffer  response_buf;
  ASSERT_EQ(rdm_buf_pack_response(&supported_params.cmd_header, 0, supported_params.resp_data(),
  ...
  EXPECT_EQ(rdm_buf_append_parameter_data(nullptr, additional_pd.data(), static_cast<uint8_t>(additional_pd.size())),
            kEtcPalErrInvalid);
  EXPECT_EQ(rdm_buf_append_parameter_data(&response_buf, nullptr, static_cast<uint8_t>(additional_pd.size())),
            kEtcPalErrInvalid);
  EXPECT_EQ(rdm_buf_append_parameter_data(&response_buf, additional_pd.data(), 0), kEtcPalErrInvalid);
}

TEST(Message, BufAppendParamaterDataTooLong)
{
  const auto supported_params = rdmtest::GetSupportedParams();
  ...
  EXPECT_EQ(
      rdm_buf_append_parameter_data(&response_buf, additional_pd.data(), static_cast<uint8_t>(additional_pd.size())),
      kEtcPalErrBufSize);
}

TEST(Message, BufAppendParameterData)
{
  const auto supported_params = rdmtest::GetSupportedParams();
  RdmBuffer  response_buf;

  ASSERT_EQ(rdm_buf_pack_response(&supported_params.cmd_header, 6, supported_params.resp_data(),
                                  supported_params.resp_data_size(), &response_buf),
            kEtcPalErrOk);
  ASSERT_EQ(response_buf.data_len, supported_params.first_packed_resp_size());
  ASSERT_EQ(
  ...
  ASSERT_EQ(rdm_buf_append_parameter_data(&response_buf, additional_pd.data(), kAdditionalPdSize), kEtcPalErrOk);
...
TEST(Message, BufUnpackCommand)
{
  ...
  EXPECT_EQ(rdm_buf_unpack_command(&packed_cmd, &cmd_header, &param_data, &param_data_len), kEtcPalErrOk);
  EXPECT_EQ(cmd_header, supported_params.cmd_header);
  EXPECT_EQ(param_data, nullptr);
  EXPECT_EQ(param_data_len, 0);
}

TEST(Message, BufUnpackCommandWithData)
{
...
TEST(Message, BufUnpackResponse)
{
  const auto reset_device = rdmtest::SetResetDevice();
  ...
  EXPECT_EQ(rdm_buf_unpack_response(&packed_resp, &resp_header, &resp_data, &resp_data_len), kEtcPalErrOk);
  EXPECT_EQ(resp_header, reset_device.resp_header);
  EXPECT_EQ(resp_data, nullptr);
  EXPECT_EQ(resp_data_len, 0);
}

TEST(Message, BufUnpackResponseWithData)
{
  const auto supported_params = rdmtest::GetSupportedParams();
